...
def get_raw_datasets(train_split=0.8, val_split=0.1, test_split=0.1) -> Dict[str, tf.data.Dataset]:
	"""Returns datasets as tf.data.Dataset, organized by split."""
	# ds_builder = tfds.builder('ogbg_molpcba')

	assert np.allclose([train_split+val_split+test_split], [1.0])

	ds_builder = tfds.builder('jet_dataset')
...
def get_datasets(batch_size: int,
								 ...
								 add_self_loops: bool = False) -> Dict[str, tf.data.Dataset]:
	...
	for split_name, dataset_split in datasets.items():
		...
		# Repeat and shuffle the data split.
		if split_name == 'train':
...
def convert_to_graphs_tuple(graph: Dict[str, tf.Tensor],
	...
	nodes = tf.cast(graph['node_feats'], dtype=tf.float32)
	labels = graph['label']
	senders = graph['edge_index'][0, :]
	receivers = graph['edge_index'][1, :]
	num_nodes = graph['num_nodes']
	num_edges = graph['num_edges']
	edges = tf.cast(graph['edge_feats'], dtype=tf.float32)
